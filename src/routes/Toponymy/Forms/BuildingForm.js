...
class BuildingForm extends Component {
  ...
  onPrint_dmymm() {
    if (this.state.saveBtnClicked) {
      print_dmymm([this.state.entity.ID], 'BuildingDM');
    } else {
  ...
  onPrint_dmhzs() {
    if (this.state.saveBtnClicked) {
      print_dmhzs([this.state.entity.ID], 'BuildingDM');
    } else {
      notification.warn({ description: '请先保存，再操作！', message: '警告' });
    }
  }
  ...
  async getFormData(id) {
    ...
    if (id) {
      ...
      let rt = await Post(url_SearchBuildingDMByID, { id: id });
      rtHandle(rt, d => {
  ...
  async delete(obj, XMWH) {
    await Post(url_DeleteBuildingDM, { ID: obj.ID, XMWH: XMWH }, e => {
  ...
  async getNameCheck(namep, name) {
    const rt = await Post(url_BuildingNameDM, {
      NameP: namep,
      Name: name,
    });
    return rt;
