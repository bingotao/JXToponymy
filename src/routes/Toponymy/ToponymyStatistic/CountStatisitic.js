...
class CountStatisitic extends Component {
  ...
  // 行政区 镇街道 地名类别 业务类型（ 地名预命名 地名命名 地名更名 地名销名 地名换补 ） 批复时间
  columns = [
    ...
    { title: '地名类别', align: 'center', dataIndex: 'DMLB', key: 'DMLB' },

    { title: '地名预命名', align: 'center', dataIndex: 'DMYMM', key: 'DMYMM' },
    { title: '地名命名', align: 'center', dataIndex: 'DMMM', key: 'DMMM' },
    { title: '地名更名', align: 'center', dataIndex: 'DMGM', key: 'DMGM' },
    { title: '地名销名', align: 'center', dataIndex: 'DMXM', key: 'DMXM' },
    { title: '地名换补', align: 'center', dataIndex: 'DMHB', key: 'DMHB' },
    ...
    return (
      <div className={st.CountStatisitic}>
        <div className={st.condition}>
          ...
          >
            <Select.Option value={'1'}>居名点</Select.Option>
            <Select.Option value={'2'}>建筑物</Select.Option>
            <Select.Option value={'3'}>道路街巷</Select.Option>
            <Select.Option value={'4'}>桥梁</Select.Option>
          </Select>
          &emsp;
          <Select
            allowClear
            style={{ width: 150 }}
            placeholder="业务类型"
            value={YWType || undefined}
            onChange={e => {
              this.condition.YWType = e;
              this.setState({
                YWType: e,
              });
            }}
          >
            <Select.Option value={'1'}>地名预命名</Select.Option>
            <Select.Option value={'2'}>地名命名</Select.Option>
            <Select.Option value={'3'}>地名更名</Select.Option>
            <Select.Option value={'4'}>地名销名</Select.Option>
            <Select.Option value={'5'}>地名换补</Select.Option>
          </Select>
          &emsp;
          <DatePicker
            onChange={e => {
        ...
        <div className={st.result}>
          <div className={st.table}>
            ...
            <DataGrid data={rows} style={{ height: '100%' }}>
              ...
              {/* 地名预命名 地名命名 地名更名 地名销名 地名换补 */}
              <GridColumn field="MPSQ" title="地名预命名" align="center" width={160} />
              <GridColumn field="MPBG" title="地名命名" align="center" width={160} />
              <GridColumn field="MPHB" title="地名更名" align="center" width={160} />
              <GridColumn field="MPZX" title="地名销名" align="center" width={160} />
              <GridColumn field="DMZM" title="地名换补" align="center" width={160} />
          ...
          <div className={st.footer}>
            ...
            地名预命名：<span>{mpsqTotal}</span>次， 地名命名：<span>{mpbgTotal}</span>次，
            地名更名：<span>{mphbTotal}</span>次， 地名销名：<span>{mpzxTotal}</span>次，
            地名换补：<span>{dmzmTotal}</span>次
          </div>
        </div>
      </div>
    );
